...
int main(int argc, char** argv)
{
    ...
    if (argc == 1)
    {
        generate_random_points(points, intensity.data(), N_point, L);

        // emission(
        //     points, intensity.data(), N_point,
        //     ATsq, d, epsilon, absorption_cross_section, exposure_time);
        emission(points, intensity.data(), N_point);
        ...
        cmos_detection(data.data(), data.data(), data.size());
        // emccd_detection(data.data(), data.data(), data.size());
        save_data("result.txt", data.data(), data.size());
    }
    else
    {
